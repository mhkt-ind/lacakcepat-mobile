...
import kotlinx.android.synthetic.main.item_news_trend.view.*
import java.text.SimpleDateFormat
import java.util.*


class NewsAdapterTrend(
    private var list: List<Article> = arrayListOf(),
    val listener: (Article) -> Unit
) :
    RecyclerView.Adapter<NewsAdapterTrend.NewsViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NewsViewHolder {
        return NewsViewHolder(
            LayoutInflater.from(parent.context).inflate(
                R.layout.item_news_trend,
                parent, false
            )
        )
    }
    ...
    class NewsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun setItem(article: Article) {
            ...
            itemView.tv_newsDesc.text = article.description
