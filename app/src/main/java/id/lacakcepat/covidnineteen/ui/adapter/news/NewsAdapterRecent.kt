...
import kotlinx.android.synthetic.main.item_news_recent.view.*
import java.text.SimpleDateFormat
import java.util.*


class NewsAdapterRecent(
    private var list: List<Article> = arrayListOf(),
    val listener: (Article) -> Unit
) :
    RecyclerView.Adapter<NewsAdapterRecent.NewsViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NewsViewHolder {
        return NewsViewHolder(
            LayoutInflater.from(parent.context).inflate(
                R.layout.item_news_recent,
                parent, false
            )
        )
    }
