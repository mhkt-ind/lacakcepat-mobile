    ...
    companion object {
        ...
        fun getBluetoothAddress(): String {
            ...
            if (address == FAKE_MAC_ADDRESS && Build.VERSION.SDK_INT < Build.VERSION_CODES.O /* Oreo */) {
                ...
                val bluetoothManagerService: Any = Mirror().on(bluetoothAdapter).get().field("mService")
                val internalAddress: Any = Mirror().on(bluetoothManagerService).invoke().method("getAddress").withoutArgs()
